	...
	<div class="container my-3">
							...
							<tr>
								...
								<td class="text-center"><a
									...
									style="text-decoration: none; color: #476268;"><c:out
											value="${reviewList.rv_title}" /></a></td>
								<td class="text-center"><fmt:formatDate
										value="${reviewList.rv_date}" pattern="yyyy-MM-dd" /></td>
								<td class="text-center"><c:out
										value="${reviewList.user_name}" /></td>
								<td class="text-center">${reviewList.rv_warning}</td>
								<td><button type="button" class="btn btn-outline-success"
										onclick="location.href='/manager/rvDelete?rv_no=${reviewList.rv_no}'">삭제</button></td>
							</tr>
						</c:forEach>
					</tbody>
				</table>
						...
						<ul class="pagination justify-content-end">
							<li class="page-item"><c:if test="${prev}">
									<a
										...
										href="/manager/reviewList?num=${reviewStartPageNum - 1}${rv_searchTypeKeyword}"
							...
							<c:forEach begin="${reviewStartPageNum}"
								...
								<span> <c:if test="${reviewSelect != num}">
										<li class="page-item"><a
											class="page-link rounded-0 mr-3 shadow-sm border-top-0 border-left-0 text-dark"
											href="/manager/reviewList?num=${num}${rv_searchTypeKeyword}">${num}</a>
										</li>
							...
							<li class="page-item"><c:if test="${next}">
									<a
										class="page-link rounded-0 mr-3 shadow-sm border-top-0 border-left-0 text-dark"
										href="/manager/reviewList?num=${reviewEndPageNum + 1}${rv_searchTypeKeyword}"
		...
		<script>
			document.getElementById("reviewSearchBtn").onclick = function() {
				...
				location.href = "/manager/reviewList?num=1"
