					...
					<tbody>
						<c:forEach items="${qaList}" var="qaList">
							<tr>
								...
								<td class="text-center">${qaList.qa_warning}</td>
								<td><button type="button" class="btn btn-outline-success"
										onclick="location.href='/manager/qaDelete?qa_no=${qaList.qa_no}'">삭제</button></td>
							</tr>
						</c:forEach>
					</tbody>
				</table>
						...
						<ul class="pagination justify-content-end">
							<li
								class="page-item rounded-0 mr-3 shadow-sm border-top-0 border-left-0 text-dark"><c:if
									test="${prev}">
									<a class="page-link"
										href="/manager/qaList?num=${qaStartPageNum - 1}${qa_searchTypeKeyword}"
							...
							<c:forEach begin="${qaStartPageNum}" end="${qaEndPageNum}"
								...
								<span> <c:if test="${qaSelect != num}">
										<li class="page-item"><a
											...
											href="/manager/qaList?num=${num}${qa_searchTypeKeyword}">${num}</a>
										</li>
							...
							<li class="page-item"><c:if test="${next}">
									<a
										class="page-link rounded-0 mr-3 shadow-sm border-top-0 border-left-0 text-dark"
										href="/manager/qaList?num=${qaEndPageNum + 1}${qa_searchTypeKeyword}"
